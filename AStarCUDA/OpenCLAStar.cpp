...
void findByAStar( GraphData* graph, int src_vert, int goal_vert){
    ...
    commandQueue = clCreateCommandQueue(context, getMaxFlopsDev(context), 0, &errNum );
    ...
    //size_t globalWorkSize = (remainder==0)? graph->vertexCount : graph->vertexCount + maxWorkGroupSize - remainder;
    ...
    allocateOCLBuffers( context, commandQueue, graph, costArray, visitedArray1,
                        ...
                        //&priorityQueueArrayDevice,
                        &visitedArrayDevice, 
                        &gCostArrayDevice, &hCostArrayDevice, &fCostArrayDevice,
                        &temporaryListDevice,&temporaryFCostDevice);
    ...
    errNum |= clSetKernelArg(astarKernel, 9, sizeof(cl_mem), &temporaryListDevice);
    errNum |= clSetKernelArg(astarKernel, 10, sizeof(cl_mem), &temporaryFCostDevice);
    errNum |= clSetKernelArg(astarKernel, 12, sizeof(int), &goal_vert);
    errNum |= clSetKernelArg(astarKernel, 13, sizeof(int), &localWorkSize);
