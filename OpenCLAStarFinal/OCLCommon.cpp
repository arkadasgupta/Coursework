...
void allocateOCLBuffers(cl_context gpuContext, cl_command_queue commandQueue, GraphData *graph,
                        ...
                        cl_mem *priorityQueueArrayDevice,cl_mem *visitedArrayDevice, 
                        cl_mem *gCostArrayDevice, cl_mem *hCostArrayDevice,cl_mem *fCostArrayDevice,
                        cl_mem *temporaryListDevice,cl_mem* temporaryFCostDevice,cl_mem* solnFoundDevice,
...
//                                           sizeof(float) * 8, costArray, &errNum);
    ...
    *latitudeArrayDevice = clCreateBuffer(gpuContext, CL_MEM_READ_ONLY, sizeof(float) * graph->vertexCount, NULL, &errNum);
    *longitudeArrayDevice = clCreateBuffer(gpuContext, CL_MEM_READ_ONLY, sizeof(float) * graph->vertexCount, NULL, &errNum);
    *priorityQueueArrayDevice = clCreateBuffer(gpuContext, CL_MEM_READ_WRITE, sizeof(int) * globalWorkSize, NULL, &errNum);
    *visitedArrayDevice = clCreateBuffer(gpuContext, CL_MEM_READ_WRITE, sizeof(int) * globalWorkSize, NULL, &errNum);
    *gCostArrayDevice = clCreateBuffer(gpuContext, CL_MEM_READ_WRITE, sizeof(float) * globalWorkSize, NULL, &errNum);
    *hCostArrayDevice = clCreateBuffer(gpuContext, CL_MEM_READ_WRITE, sizeof(float) * globalWorkSize, NULL, &errNum);
    *fCostArrayDevice = clCreateBuffer(gpuContext, CL_MEM_READ_WRITE, sizeof(float) * globalWorkSize, NULL, &errNum);
    *temporaryListDevice = clCreateBuffer(gpuContext, CL_MEM_READ_WRITE, sizeof(int) * 8, NULL, &errNum);
    *temporaryFCostDevice = clCreateBuffer(gpuContext, CL_MEM_READ_WRITE, sizeof(float) * 8, NULL, &errNum);
    *solnFoundDevice = clCreateBuffer(gpuContext, CL_MEM_READ_WRITE, sizeof(int) , NULL, &errNum);
